...
@Component({
  selector: 'app-component-registration-type',
  templateUrl: './registrationType.component.html'
})
export class RegistrationTypeComponent implements AfterViewInit{
  data: RegistrationTypeModel[];
  confirm: ConfirmModel;
  formPop: PopUpModel;
  selectedItem: RegistrationTypeModel;
  ...
  Edit(): void {
    const item: RegistrationTypeModel = this.GetSelectedItem();
  ...
  Save(model: RegistrationTypeModel): void {
    this.Blocker.block = true;

    if (this.data.filter(x => x.id === model.id).length === 0) {
  ...
  private SaveData(item: RegistrationTypeModel): void {
  ...
  private UpdateData(item: RegistrationTypeModel): void {
  ...
  private DeleteData(item: RegistrationTypeModel): void {
  ...
  private GetSelectedItem(): RegistrationTypeModel {
